...
export function getUpcomingCloseTime(place, now = new Date()) {
    ...
    const currentPeriod = getCurrentPeriod(place.openingHours, place.utcOffsetMinutes, now);
...
export function isOpen(place, now = new Date()) {
    ...
    const { period } = getCurrentPeriod(place.openingHours, place.utcOffsetMinutes, now);
    return !!period;
}
